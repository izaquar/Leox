...
class cleoController:
    ...
    def colours_menu(self,parent, p):
        ...
        parent.add_command(label='New Color', underline=0,
            command=cleoAddColorsMenuCallback)
            
        def cleoColorsMenuCallback():
            self.remove_colours(p.v)
    
        parent.add_command(label='Clear Color', underline=0,
    ...
    def show_menu (self,tag,k):
        ...
        menu.add_command(label='T',underline=0,command=lambda:self.set_time_req(p))
        
        menu.add_command(label='Find next todo', columnbreak=1,underline=0,command=lambda:self.find_todo(p))
        ...
        # fonts_menu(menu,p)
        menu.add_separator()
        menu.add_command(label='Clear All for node',underline=0,command=lambda:self.clear_all(v))
        menu.add_command(label='Flush empty cleo attribs.',underline=0,command=self.dropEmptyAll)
